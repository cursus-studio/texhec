...
func (s *service) NewBiomAsset(srcImages [6][]image.Image) (tile.BiomAsset, error) {
	...
	for _, img := range srcImages[0] {
		...
		dstImages[11] = append(dstImages[11], s.Graphics().NewImage(img).FlipV().Image())
	}
	dstImages[14] = append(dstImages[14], srcImages[1]...)
	for _, img := range srcImages[2] {
		dstImages[6] = append(dstImages[6], s.Graphics().NewImage(img).FlipV().Image())
		dstImages[10] = append(dstImages[10], s.Graphics().NewImage(img).FlipHV().Image())
		dstImages[12] = append(dstImages[12], img)
		dstImages[13] = append(dstImages[13], s.Graphics().NewImage(img).FlipH().Image())
	}
	for _, img := range srcImages[3] {
		dstImages[4] = append(dstImages[4], img)
		dstImages[9] = append(dstImages[9], s.Graphics().NewImage(img).FlipH().Image())
	}
	for _, img := range srcImages[4] {
		dstImages[5] = append(dstImages[5], img)
		dstImages[8] = append(dstImages[8], s.Graphics().NewImage(img).FlipH().Image())
	}
	for _, img := range srcImages[5] {
		dstImages[0] = append(dstImages[0], s.Graphics().NewImage(img).FlipH().Image())
		dstImages[1] = append(dstImages[1], img)
		dstImages[3] = append(dstImages[3], s.Graphics().NewImage(img).FlipHV().Image())
