...
type Setup struct {
	W       ecs.World
	Array   ecs.ComponentArray[Component]
...
func NewSetup() Setup {
	c := ioc.NewContainer(
		...
		relationpkg.SpatialRelationPkg(
			func(w ecs.World) ecs.DirtySet {
				...
				ecs.GetComponentArray[Component](w).AddDirtySet(dirtySet)
				return dirtySet
			},
			func(w ecs.World) func(entity ecs.EntityID) (uint32, bool) {
				componentArray := ecs.GetComponentArray[Component](w)
	...
	return Setup{
		W:       w,
		Array:   ecs.GetComponentArray[Component](w),
